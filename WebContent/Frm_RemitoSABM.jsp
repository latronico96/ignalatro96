...
<%
	...
	String idForm = "Frm_RemitoSABM";
	String idGrilla = "RemitoSABM";
	String URL = "'./Frm_RemitoSABM'";
...
<div id="<%=idForm%>" data-popup="true" class="backmodal formulario">
	...
	<script type="text/javascript">       
			...
			modoGrilla:"ALTA",
       		GetClienteRow: function (nrocli=""){       			
            	if(nrocli.toString().trim()==""){
            		...
            		rowCli=JSON.parse(valyget("CLI",nrocli));    
            		switch(rowCli.error){
    	                case "2":
    	                	abrirAlerta("ERR", "Error al tratar de buscar el Cliente","Error interno");
    	            		$("#rem_codcl",formulario).val(""); 
    	                	return "";
    		                break;
    	                case "1":
    	                	abrirAlerta("NOT", "No se encontro el Cliente");
    	            		$("#rem_codcl",formulario).val(""); 
    	                	return "";
    		                break;
    	                case "0":
       	 	...
       	 	CargarCliente: function (rowCli={}){
            	$(".cli_campo",this).each(function(){
            		$(this).val(rowCli[$(this).attr('id')]);
            	});
            	if (this.modo=="ALTA" || this.modo=="MODI"){
            		$("#rem_condi",formulario).val(rowCli.cli_condi);
            	}            	
            },       		
        ...
        $(document).ready(function(){
        	...
        	$("#btn_HcCli",formulario).unbind("click").click(function(){
            	HelpCampo("CLI","#btn_HcCli",function(row,id){
            				$("#rem_codcl",formulario).val(id);         				
            				formulario.CargarCliente(row); 
            			});
        	});
        	$("#rem_codcl",formulario).unbind("change").change(function(){
        		formulario.CargarCliente(formulario.GetClienteRow(this.value)); 
        	});
        	        	
        	formulario.Grilla();
        	 
         	$("#btn_hcArt",formulario).unbind("click").click(function(){
            	HelpCampo("ART","#btn_HcCli",function(row,id){         				
